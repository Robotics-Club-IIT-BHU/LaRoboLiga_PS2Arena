...
class LaRoboLigaPs2Arena():
    ...
    def open_husky_gripper(self):
        """
            Function to open the grippers attached in from of husky
    ...
    def get_camera_image(self):
        ...
        images = p.getCameraImage(self._width,
                            self._height,
                            view_matrix,
                            projection_matrix,
                            shadow=True,
    ...
    def load_balls(self, custom_ball_locations = None):
        """
    ...
    def __load_arena(self):
        """
            Function to load the arena and the car

            Arguments:
                    custom_ball loacations : 
                                    Read the string docs in "load_balls" function
...
if __name__ == '__main__':
    color_balls_location = dict({
                'red'    : [6,6,2.0],
                'yellow' : [-6,6,2.0],
                'blue'   : [-6,-6,2.0],
                'purple' : [6,-6,2.0]
            })
    env = LaRoboLigaPs2Arena(ball_locations=color_balls_location,husky_pos=[4,4,0.5])
    while True:
        while i>2000 and i<2500:
            env.move_husky(3.14,3.14,3.14,3.14)
            p.stepSimulation()
            i+=1
